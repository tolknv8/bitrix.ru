        ...
        <? foreach ($arResult["ITEMS"] as $cell => $arElement): ?>
            ...
            <td valign="top" width="<?= round(100 / $arParams["LINE_ELEMENT_COUNT"]) ?>%"
                ...
                <? if ($arElement["CAN_BUY"]): ?>
                                            ...
                                            <? if (
                                                ...
                                                <? foreach ($product_property["VALUES"] as $k => $v): ?>
                                                    <label><input type="radio"
                                                                  ...
                                                                  value="<? echo $k ?>" <? if ($k
                                                            == $product_property["SELECTED"])
                                                            echo '"checked"' ?>><? echo $v ?></label><br>
                                                <? endforeach; ?>
                                            <? else: ?>
                                                <select
                                                    name="<? echo $arParams["PRODUCT_PROPS_VARIABLE"] ?>[<? echo $pid ?>]">
                                                    <? foreach ($product_property["VALUES"] as $k => $v): ?>
                                                        <option value="<? echo $k ?>" <? if ($k
                                                            == $product_property["SELECTED"])
                                                            echo '"selected"' ?>><? echo $v ?></option>
                                                    <? endforeach; ?>
                                                </select>
                                            <? endif; ?>
                                        </td>
                    ...
                    <? else: ?>
                        <noindex>
                            <a href="<? echo $arElement["BUY_URL"] ?>" rel="nofollow"><? echo GetMessage(
                                    "CATALOG_BUY"
                                ) ?></a>&nbsp;<a href="<? echo $arElement["ADD_URL"] ?>"
