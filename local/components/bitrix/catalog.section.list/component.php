...
$arParams["ADD_SECTIONS_CHAIN"] =
    !(isset($arParams["ADD_SECTIONS_CHAIN"]) && $arParams["ADD_SECTIONS_CHAIN"] === "N"); //Turn on by default
...
if ($this->startResultCache(
    false,
    [$arrFilter, ($arParams["CACHE_GROUPS"] === "N" ? false : $USER->GetGroups())]
)) {
    ...
    if (is_array($arResult["SECTION"])) {
        ...
        while ($arPath = $rsPath->GetNext()) {
            if ($arParams["ADD_SECTIONS_CHAIN"]) {
                $ipropValues                =
                    new \Bitrix\Iblock\InheritedProperty\SectionValues($arParams["IBLOCK_ID"], $arPath["ID"]);
    ...
    $elementCountFilter = [
        ...
        "CHECK_PERMISSIONS"   => "Y",
        "MIN_PERMISSION"      => "R",
        "INCLUDE_SUBSECTIONS" => (isset($sectionFilter["ELEMENT_SUBSECTIONS"])
        && $sectionFilter["ELEMENT_SUBSECTIONS"] == "N" ? "N" : "Y"),
    ];
    if (!empty($sectionFilter['PROPERTY']) && is_array($sectionFilter['PROPERTY'])) {
        foreach (array_keys($sectionFilter['PROPERTY']) as $propertyId) {
            $field                                                                =
                CIBlock::MkOperationFilter($propertyId);
            $elementCountFilter[$field['PREFIX'] . 'PROPERTY_' . $field['FIELD']] =
                $sectionFilter['PROPERTY'][$propertyId];
        }
    ...
    if ($arParams["COUNT_ELEMENTS"] && $arResult['SECTION']['ID'] > 0) {
        ...
        $arResult["SECTION"]["~ELEMENT_CNT"] = CIBlockElement::GetList([], $elementFilter, []);
    ...
    foreach ($arResult["SECTIONS"] as &$arSection) {
        $ipropValues                   =
            new \Bitrix\Iblock\InheritedProperty\SectionValues($arSection["IBLOCK_ID"], $arSection["ID"]);
        ...
        if ($arParams["COUNT_ELEMENTS"]) {
            ...
            $arSection["~ELEMENT_CNT"] = CIBlockElement::GetList([], $elementFilter, []);
...
if ($arResult["SECTIONS_COUNT"] > 0 || isset($arResult["SECTION"])) {
    if (
        ...
        $arButtons   = CIBlock::GetPanelButtons(
            ...
            0,
            $arResult["SECTION"]["ID"],
            ["RETURN_URL" => $arReturnUrl, "CATALOG" => true]
        );
